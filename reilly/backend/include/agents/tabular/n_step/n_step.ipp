...
NStep::NStep(size_t states, size_t actions, float alpha, float epsilon, float gamma, size_t n_step, float epsilon_decay)
    : TabularAgent(states, actions, alpha, epsilon, gamma, epsilon_decay), n_step(n_step) {}

NStep::NStep(const NStep &other) : TabularAgent(other), n_step(other.n_step), T(other.T), trajectory(other.trajectory) {}
